...
const string strMessageMagic = "MyX Signed Message:\n";
...
{
    const int nMinSubsidy = 0.069069 * COIN;
	int64_t nSubsidy = 0 * COIN;
    if(nHeight == 1) {
    nSubsidy = 5000000 * COIN; 
    } else if(nHeight >= 15 && nHeight <= 200015){ 
    nSubsidy = 300 * COIN; 
    } 
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in MyX
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MyX", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "NOTHING TO SEE HERE BESIDES MYX";
        CTransaction txNew;
        txNew.nTime = 1513296567;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print        
        block.print();
        assert(block.hashMerkleRoot == uint256("0x6c7806cf5ec8baea1a529a81ffbf9f79f6e2c778877071fd58bb0b221020abc2"));
