...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #myxTEST2\r");
            Send(hSocket, "WHO #myxTEST2\r");
        } else {
            // randomly join #myx00-#myx05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #myx%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #myx%02d\r", channel_number).c_str());
        }
